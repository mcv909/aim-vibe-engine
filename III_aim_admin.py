...
# --- 🛰️ AIM KONFIGURATION (FEINKALIBRIERT FÜR QWEN2 INSTRUCT) ---
AIM_CONFIG = {
    "VALUE_MATCH_MIN": 0.28,     # 🟨 Ab 0.28: Das echte Resonanz-Feld (Potenzial / 50/50)
    "DISMATCH_VETO": 0.25,       # 🟥 Unter 0.25: Harter Gegenpol
    "FINAL_RESONANCE_MIN": 0.31, # 🔥 Ab 0.31: Die echten 100% Matches!
    "WEIGHTS": {
        "werte": 0.40,
        "general": 0.20,
        "vibe": 0.15,
...
def get_expected_status(email):
    try:
        ...
        # 🛠️ Entfernt _2 oder _3, damit unsere Klone richtig erkannt werden
        name = name.split('_')[0] 
...
def get_score_visual(score):
    norm_score = min(1.0, max(0.0, score / 0.60)) 
...
def analyze_user_kaskade(email):
    ...
    try:
        ...
        for r in results:
            ...
            # 🚦 DAS NEUE 3-STUFEN RADAR
            if r['final'] >= AIM_CONFIG["FINAL_RESONANCE_MIN"]:
                ist = "🔥 MATCH"
            elif r['final'] >= AIM_CONFIG["VALUE_MATCH_MIN"]:
                ist = "🌊 RESONANZ"
            else:
                ist = "☁️ DISSONANZ"
